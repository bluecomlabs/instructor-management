  ...
  setup() {
    ...
    const changeProgramStatus = async () => {
      ...
      try {
        ...
        console.log("API 호출 URL:", ApiUrl(`/admin/programs/status`));
        console.log("요청 바디:", requestBody);

        await axios.post(
          ApiUrl(`/admin/programs/status`),
          requestBody,
          {
            headers: {
    ...
    const fetchPrograms = async (
      ...
      try {
        ...
        const response = await axios.get(
          ApiUrl(`/admin/programs?page=${page}&size=${pageSize.value}&search=${search.value}${sortBy}${filterQuery}`),
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
