  ...
  <div class="card">
    <div class="card-header border-0 pt-6 cmcard-header">
      <div class="d-flex align-items-center me-3">
        <div class="d-flex align-items-center">
          <select :class="{ 'cmdel-selected': selectedIds.length > 0 }" v-model="filterGoalIsConfirmed" class="form-select filtercheckbox-button dropdown-button">
          ...
          <select
            ...
            class="form-select filtercheckbox-button dropdown-button"
            :class="{ 'cmdel-selected': selectedIds.length === 0 }"
          >
            <option value="UPDATE">신청하기</option>
            <option value="DELETE">취소하기</option>
          </select>
    ...
    <div class="card-body pt-0">
      ...
      >
        ...
        <template v-slot:application="{ row: customer }">
          <div class="column-application">
            <button
              @click="onApplicationClick(customer)"
              :class="['btn', customer.application === 'Y' ? 'btn-danger' : 'btn-primary']"
            >
              {{ customer.application === 'Y' ? '취소하기' : '신청하기' }}
            </button>
          </div>
        </template>
      </KTDatatable>
...
export default defineComponent({
  ...
  setup() {
    ...
    const onApplicationClick = async (customer: IProgram) => {
      const token = localStorage.getItem("token");
      const educationIds = [customer.id];
